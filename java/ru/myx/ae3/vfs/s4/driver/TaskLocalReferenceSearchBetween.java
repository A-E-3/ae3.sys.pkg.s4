...
final class TaskLocalReferenceSearchBetween //
		extends
	...
	TaskLocalReferenceSearchBetween(//
	...
	public String toString() {
		...
		return "TASK_REF_SEARCH_BETWEEN(" + Format.Compact.toPeriod(System.currentTimeMillis() - this.started) + "): " //
				+ this.record //
	...
	protected void execute(final S4WorkerContext context) throws Exception {
		...
		final int limit;
		if (leftTotal == 0 || leftTotal > TaskLocalReferenceSearchBetween.LONG_THRESHOLD) {
			limit = this.leftCurrent = TaskLocalReferenceSearchBetween.LONG_ITERATION + 1;
		} else {
			limit = leftTotal;
			this.leftCurrent = 0;
		}
